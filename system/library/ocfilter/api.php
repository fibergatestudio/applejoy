...
class Api extends Factory {
  ...
  public function setProductSQL($fn, &$sql = '') {
    ...
    $call_key = $fn . md5(json_encode(array_column(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 4), 'file')));
    
    if (isset(self::$temp['product.sql.' . $call_key])) {
      return;
    }
    ...
    */
    ...
    self::$temp['product.sql.' . $call_key] = true;    
  }  
  ...
  public function setProductListControllerData(&$data, $product_total = null) {
    ...
    $data['ocfilter_mobile_button'] = (!empty($this->config('mobile_button_position')) && ($this->config('mobile_button_position') == 'static' || $this->config('mobile_button_position') == 'both'));
    ...
    if ($this->params->get()) {
      ...
      $data['description'] = $data['description_bottom'] = $data['description_top'] = $data['description_2'] = $data['ext_description'] = '';      
      ...
      if (isset($this->opencart->request->get['path']) && $this->isSeoPage()) {
        ...
        if ($page == 1) {
          ...
          $this->opencart->document->addLink($this->opencart->url->link('product/category', 'path=' . $this->opencart->request->get['path'] . '&' . $this->params->getIndex() . '=' . $this->seo->getParams() . '&page='. $page, 'SSL'), 'canonical');
        }
