...
class Seo extends Factory {
  ...
  public function link($params = '') {
    ...
    if ($this->placement->isCustomPage()) {
      if ($params || !isset($this->opencart->request->get['ocf_custom_route'])) {
        ...
        $link = $this->opencart->url->link('product/search', $url);        
      ...
      $link = $this->opencart->url->link($this->route, $url);
    } else {
      $link = $this->opencart->url->link('product/category', $url);
    }
  ...
  public function formatPage($result) {
    ...
    if (isset($this->rewrite_cache[$cache_key])) {
      ...
      $link = $this->opencart->url->link('product/category', 'path=' . $result['path']);
  ...
  public function formatPages($pages = [], $filter_data = []) {
    ...
    foreach ($pages as $page) {
      ...
      if (isset($this->rewrite_cache[$cache_key])) {
        ...
        $link = $this->opencart->url->link('product/category', 'path=' . $page['path']);
        ...
        if (isset($values['min']) && isset($values['max'])) {
          $has_param = (
            isset($product_values['min']) && isset($product_values['max']) && (
              ($values['min'] <= $product_values['min'] && $values['max'] >= $product_values['max']) ||
              ($product_values['min'] <= $values['min'] && $product_values['max'] >= $values['max'])
            )
          );
        } else {
