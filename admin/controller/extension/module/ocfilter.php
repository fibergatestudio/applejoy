...
class ControllerExtensionModuleOCFilter extends Controller { 
  ...
  public function getCopyLog() {
    ...
    $this->ocfilter->opencart->responseTEXT('OCFilter log is empty');
  }
  ...
  protected function checkInstall() {
    ...
    if (version_compare(VERSION, '3.0', '>=')) {
      ...
      if (!in_array('ocfilter_api', array_column($this->model_setting_event->getEvents(), 'code'))) {
        $this->installEvent();
      }
    } else {
      $this->load->model('extension/event');
      
      if (!$this->model_extension_event->getEvent('ocfilter_api', 'catalog/view/*/before', 'extension/module/ocfilter/eventApi')) {
        $this->installEvent();
      }
    }
