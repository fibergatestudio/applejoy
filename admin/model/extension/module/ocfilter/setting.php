...
class ModelExtensionModuleOCFilterSetting extends Model {   
  public function setSitemapKeyword() {
    if ($this->ocfilter->opencart->version >= 30) {
      ...
      if (!$query->num_rows) {               
        foreach ($this->ocfilter->admin->getStores() as $store_id) {
          foreach ($this->ocfilter->admin->getLanguages() as $language) {
  ...
  public function upgradeFrom47() {
    ...
    foreach ($query->rows as $result) {
      ...
      if ($params) {
        ...
        if ($this->ocfilter->opencart->version >= 30) {
          foreach ($this->ocfilter->admin->getLanguages() as $key => $language) {
            foreach ($this->ocfilter->admin->getStores() as $store_id) {
