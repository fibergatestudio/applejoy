...
class ModelExtensionModuleOCFilterFilter extends Model {
  ...
  */
  ...
  public function setOCFilterFilter($product_id, $data) {
    ...
    if (isset($data['ocfilter_filter'])) {
      foreach ($data['ocfilter_filter'] as $filter_key => $values) {       
        ...
        if (isset($values['min']) && isset($values['max']) && (strlen($values['min']) + strlen($values['max'])) > 0) {
          $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_filter_range_to_product SET product_id = '" . $this->db->escape((string)$product_id) . "', filter_id = '" . (int)$filter_id . "', source = '" . (int)$source . "', `min` = '" . (float)$values['min'] . "', `max` = '" . (float)$values['max'] . "'");
        } else if (is_array($values)) {
          foreach ($values as $value_id) {    
            $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_filter_value_to_product SET product_id = '" . $this->db->escape((string)$product_id) . "', filter_id = '" . (int)$filter_id . "', source = '" . (int)$source . "', value_id = '" . $this->db->escape((string)$value_id) . "'");          
          }          
        }        
      }
    }
