...
class ControllerCheckoutCart extends Controller {
	...
	public function minicart(){
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$intdiv = $total_products > 20 ? $total_products % 10 : $total_products;
				if($intdiv == 1){
					$data["text_count_product"] = $this->language->get('text_product_one');
				} else if($intdiv > 1 && $intdiv < 5){
					$data["text_count_product"] = $this->language->get('text_product_more_one');
				} else {
					$data["text_count_product"] = $this->language->get('text_more_products');
				}
			$data['cart_total'] = $total_products;
			$data["text_on_summ"] = $this->language->get('text_on_summ');
		} else {
			$data['text_error'] = $this->language->get('text_empty');
		...
		$this->response->setOutput($this->load->view('checkout/minicart', $data));
	}
}
