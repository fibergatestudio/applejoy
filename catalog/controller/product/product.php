...
class ControllerProductProduct extends Controller
{
    ...
    public function index()
    {
        ...
        if (!in_array($product_id, $_SESSION["viewed"])) {
        ...
        if ($product_info) {
            ...
            if (
                ...
                $data["special"] = $special = $this->currency->format(
                    $this->tax->calculate(
                ...
                $data[
                    "procent"
                ] = $this->model_catalog_product->procent_calculate(
                    $product_info
                );
            ...
            foreach ($results as $result) {
                ...
                if (
                    !is_null($result["special"]) &&
                    (float) $result["special"] >= 0
                ) {
            ...
            $data["text_not_in_stok"] = $this->language->get(
                "text_not_in_stok"
            );

            $data["text_buy"] = $this->language->get("text_buy");

            $data["text_buy_1_click"] = $this->language->get(
                "text_buy_1_click"
            );

            $data["notify_when_it_appears"] = $this->language->get(
                "notify_when_it_appears"
            );

            $data["text_tab_delivery"] = $this->language->get(
                "text_tab_delivery"
            );

            $data["text_tab_money"] = $this->language->get("text_tab_money");

            $data["text_tab_warranty"] = $this->language->get(
                "text_tab_warranty"
            );
            ...
            $data["text_characteristik"] = $this->language->get(
                "text_characteristik"
            );

            $data["text_descriptions"] = $this->language->get(
                "text_descriptions"
            );

            $data["tab_rel_productions"] = $this->language->get(
                "tab_rel_productions"
            );
            ...
            $data["label_wtite_review"] = $this->language->get(
                "label_wtite_review"
            );
            ...
            $data["label_review_text"] = $this->language->get(
                "label_review_text"
            );

            $data["placeholder_review"] = $this->language->get(
                "placeholder_review"
            );
            ...
            $data["recurrings"] = $this->model_catalog_product->getProfiles(
                $this->request->get["product_id"]
            );

            $this->model_catalog_product->updateViewed(
                $this->request->get["product_id"]
            );
            ...
            $data["column_right"] = $this->load->controller(
                "common/column_right"
            );

            $data["content_top"] = $this->load->controller(
                "common/content_top"
            );
    ...
    public function review()
    {
        ...
        $review_total = $this->model_catalog_review->getTotalReviewsByProductId(
            $this->request->get["product_id"]
        );
    ...
    public function write()
    {
        ...
        if ($this->request->server["REQUEST_METHOD"] == "POST") {
            ...
            if (
                utf8_strlen($this->request->post["text"]) < 25 ||
    ...
    public function getRecurringDescription()
    {
        ...
        $recurring_info = $this->model_catalog_product->getProfile(
            $product_id,
            $recurring_id
        );
    ...
    public function fastBuyPhone()
    {
        ...
        if ($this->request->post) {
            ...
            $fb_phone = isset($this->request->post["phone"])
            ...
            $fastBuyProductTotal =
                $fastBuyProductPrice * $fastBuyProductQuantity;
            ...
            $this->db->query(
                "INSERT INTO " .
                    DB_PREFIX .
                    "order_history SET order_id = '" .
                    (int) $order_id .
                    "', order_status_id = '2', notify = '0', comment = 'FAST ORDER', date_added = NOW()"
            );
            ...
            $mail->smtp_hostname = $this->config->get(
                "config_mail_smtp_hostname"
            );

            $mail->smtp_username = $this->config->get(
            ...
            $mail->smtp_timeout = $this->config->get(
            ...
            $mail->setHTML(
                "<p>Вы получили новый заказ.</p> <p>Товар: " .
                    $prd_info["name"] .
                    " (" .
                    $option_text .
                    " )</p> <p>Номер телефона клиента: " .
